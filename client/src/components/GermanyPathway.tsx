...
export default function GermanyPathway() {
  return (
    <div className="space-y-10">
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative min-h-[350px] overflow-hidden bg-[#30201b]">
            <SafeImage src="/assets/germany/german-flag.png" alt="German flag" className="absolute inset-0 h-full w-full object-cover opacity-85" loading="eager" />
        ...
        <div className="grid gap-4 border-t border-border bg-secondary/30 p-4 sm:grid-cols-[1fr_1.25fr] sm:p-5">
          <div className="overflow-hidden rounded-xl border border-border bg-card"><SafeImage src="/assets/germany/germany-states-map.jpg" alt="Map of Germany's federal states" className="h-40 w-full object-cover" /></div>
      ...
      <section id="hospitals-germany" aria-labelledby="hospitals-germany-heading"><SectionHeading eyebrow="07 / Important hospitals" title="One major hospital or university medical center in each federal state" body="This is a curated orientation list, not an official ranking and not a promise of residency placement. Always check the department's current vacancies, specialty-training authorisation, German-language expectations, and licence requirements." /><div id="hospitals-germany-heading" className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{germanyPathway.hospitals.map((hospital) => <article key={hospital.state} className="rounded-2xl border border-border bg-card p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-primary"><Building2 size={19} /></div><span className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-primary">{hospital.state}</span></div><h3 className="mt-4 text-base font-extrabold">{hospital.name}</h3><p className="mt-1 text-xs font-bold text-muted-foreground">{hospital.city}</p><p className="mt-3 text-sm leading-6 text-muted-foreground">{hospital.body}</p><a href={hospital.url} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-primary hover:underline">Official site <ArrowUpRight size={13} /></a></article>)}</div></section>
